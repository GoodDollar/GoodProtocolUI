  ...
  if (currencyAmountIn && currencyOut && allowedPairs.length > 0) {
    ...
    for (let i = 1; i <= maxHops; i++) {
      const currentTrade: Trade<Currency, Currency, TradeType.EXACT_INPUT> | null =
        Trade.bestTradeExactIn(allowedPairs, currencyAmountIn, currencyOut, { maxHops: i, maxNumResults: 3 })[0] ??
        null;

      // if current trade is best yet, save it

      console.log({ currentTrade, maxHops });
      if (isTradeBetter(bestTradeSoFar, currentTrade, BETTER_TRADE_LESS_HOPS_THRESHOLD)) {
        console.log("found trade better -->");
