...
export class SavingsWidget extends LitElement {
    ...
    static get properties() {
        return {
            ...
            connectWallet: { type: Object }, // Functional property
            activeTab: { type: String, state: true },
            inputAmount: { type: String, state: true },
            walletBalance: { type: BigInt, state: true },
            currentStake: { type: BigInt, state: true },
            unclaimedRewards: { type: BigInt, state: true },
            totalStaked: { type: BigInt, state: true },
            userWeeklyRewards: { type: BigInt, state: true },
            annualAPR: { type: Number, state: true },
            interval: { type: Object, state: true },
            isLoading: { type: Boolean, state: true },
            txLoading: { type: Boolean, state: true },
            isClaiming: { type: Boolean, state: true },
            inputError: { type: String, state: true },
            transactionError: { type: String, state: true },
        }
    }

    declare web3Provider: any
    declare connectWallet: (() => void) | undefined
    declare activeTab: string
    declare inputAmount: string
    declare walletBalance: bigint
    declare currentStake: bigint
    declare unclaimedRewards: bigint
    declare totalStaked: bigint
    declare userWeeklyRewards: bigint
    declare annualAPR: number
    declare interval: number | null
    declare isLoading: boolean
    declare txLoading: boolean
    declare isClaiming: boolean
    declare inputError: string
    declare transactionError: string
    ...
    constructor() {
        ...
        this.connectWallet = undefined
        this.activeTab = 'stake'
        this.inputAmount = '0.0'
        this.walletBalance = BigInt(0)
        this.currentStake = BigInt(0)
        this.unclaimedRewards = BigInt(0)
        this.totalStaked = BigInt(0)
        this.userWeeklyRewards = BigInt(0)
        this.annualAPR = 0
        this.interval = null
    ...
    render() {
        ...
        return html`
            <div class="container">
                ...
                <div class="input-section">
                    <div class="balance-info ${!isConnected ? 'hidden' : ''}">
                        <span>
                            ${this.activeTab === 'stake'
                                ? `Wallet Balance: ${
                                      this.isLoading ? 'Loading...' : this.formatBigInt(this.walletBalance)
                                  }`
                                : `Current Stake: ${
                                      this.isLoading ? 'Loading...' : this.formatBigInt(this.currentStake)
                                  }`}
                        </span>
                    </div>
                ...
                ${this.transactionError
                    ? html`<div class="error-message" style="margin-bottom: 16px; text-align: center;">
                          ${this.transactionError}
                      </div>`
                    : ''}
                ${!isConnected
                    ? html`
                          <button class="main-button primary" @click=${this.handleConnectWallet}>Connect Wallet</button>
                      `
                    : html`
                          <button
                              class="main-button"
                ...
                <div class="stats-section">
                    ...
                    <div class="stat-row">
                        ...
                        <span class="stat-value"
                            >${this.isLoading ? 'Loading...' : this.formatBigInt(this.totalStaked)}</span
                        >
                    </div>
                    ...
                    ${isConnected
                        ? html`
                              <div class="stat-row">
                                  ...
                                  <span class="stat-value"
                                      >${this.isLoading ? 'Loading...' : this.formatBigInt(this.currentStake)}</span
                                  >
                              </div>
                    ...
                    <div class="stat-row">
                        ...
                        <span class="stat-value"
                            >${this.isLoading ? 'Loading...' : this.formatPercent(this.annualAPR)}</span
                        >
                    </div>
                </div>
...
customElements.define('gooddollar-savings-widget', SavingsWidget)
